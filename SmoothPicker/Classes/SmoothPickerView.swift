...
open class SmoothPickerView: UIView
{
    ...
    private func commInit()
    {
        ...
        let lastItemWidth = (dataSource?.itemForIndex(index: (dataSource?.numberOfItems(pickerView: self))! - 1,
                                                      pickerView: self).frame.size.width)!
        ...
        self.sliderCollectionView?
            .register(SmoothPickerCollectionViewCell.self, forCellWithReuseIdentifier: cellIdentifier)
...
extension SmoothPickerView: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout
{
    ...
    {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "SmoothPickerCollectionViewCell",
                                                      for: indexPath) as! SmoothPickerCollectionViewCell
    ...
    {
        let size = CGSize(width: itemsWidth[indexPath.row],
                          height: (dataSource?.itemForIndex(index: indexPath.row, pickerView: self).frame.height)!)
        return size
    }

    public func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath)
    {
        sliderCollectionView?.scrollToItem(at: indexPath,
                                           at: .centeredHorizontally,
                                           animated: currentSelectedCell != nil)
...
extension SmoothPickerView: SmoothPickerScrollDelegate
{
    ...
    {
        sliderHandler?.scrollViewWillEndDragging(scrollView,
